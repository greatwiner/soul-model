...
int NgramWordTranslationDataSet::resamplingSentence(int totalLineNumber,
...
int NgramWordTranslationDataSet::addLine(ioFile* iof) {
	int i = 0;
	string line;
	string headline;
	headline = "";
...
int NgramWordTranslationDataSet::readText(ioFile* iof) {
...
int NgramWordTranslationDataSet::resamplingText(ioFile* iof,
...
int NgramWordTranslationDataSet::readTextNgram(ioFile* iof) {
	cerr << "ERROR: readTextNgram is called with NgramWordTranslationDataSet"
			<< endl;
	exit(1);
}

int NgramWordTranslationDataSet::readCoBiNgram(ioFile* iof) {
	int readLineNumber = 0;
	int i;
	int N;
	iof->readInt(N);
...
void NgramWordTranslationDataSet::writeReBiNgram(ioFile* iof) {
...
int NgramWordTranslationDataSet::addDisWordTuple(int* srcIndex, int* desIndex,
		int* unkIndex) {
	int i;
	int use = 1;
...
int tupleCompare(const void *ngram1, const void *ngram2) {
...
void NgramWordTranslationDataSet::sortNgram() {
	qsort((void*) data, (size_t) ngramNumber, (nm + 3) * sizeof(unsigned int),
			tupleCompare);

}
void NgramWordTranslationDataSet::shuffle(int times) {
	int n3 = nm + 3;
...
int NgramWordTranslationDataSet::writeReBiNgram() {
	int i;
	int ngramId = 0;
...
float NgramWordTranslationDataSet::computePerplexity() {
	perplexity = 0;
...
int NgramWordTranslationDataSet::addLine(string line) {
	cerr
