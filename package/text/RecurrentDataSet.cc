...
int RecurrentDataSet::addLine(string line) {
	int j;
	int inputIndex[MAX_WORD_PER_SENTENCE];
	int outputIndex[MAX_WORD_PER_SENTENCE];
	istringstream streamLine(line);
...
int RecurrentDataSet::resamplingSentence(int totalLineNumber,
	...
	if (totalLineNumber == resamplingLineNumber) {
		...
		if (cont) {
			...
			for (i = 1; i < resamplingLineNumber; i++) {
				resamplingLineId[i] = (resamplingLineId[0] + i)
						% totalLineNumber;
			}
		} else {
...
int RecurrentDataSet::readText(ioFile* iof) {
...
int RecurrentDataSet::resamplingText(ioFile* iof, int totalLineNumber,
	...
	for (rNBlock = 0; rNBlock < outMBlock; rNBlock++) {
		...
		for (rBlockSize = 0; rBlockSize < blockSize; rBlockSize++) {
			if (rNBlock * (n + 3) < dis(rBlockSize)) {
				...
				for (rN = 0; rN < n + 2; rN++) {
					dataTensor(rBlockSize + rNBlock * blockSize, rN) = ssArray(
							rN);
				}
			}
...
int RecurrentDataSet::readTextNgram(ioFile* iof) {
	string line;
	int readLineNumber = 0;
	while (!iof->getEOF()) {
		if (iof->getLine(line)) {
...
void RecurrentDataSet::writeReBiNgram(ioFile* iof) {
	intTensor pos;
	pos.resize(blockSize, 1);
	intTensor dis;
	dis.resize(blockSize, 1);
...
int RecurrentDataSet::writeReBiNgram() {
	...
	dis.resize(blockSize, 1);
	int outMBlock = findPos(pos, dis); // the maximum number of n-gram in each paragraph * (n + 3)
	cout << blockSize * outMBlock / (n + 3) << " ngrams" << endl;
	int rNBlock = 0;
	int rBlockSize;
	intTensor ssArray;
	ssArray.resize(n, 1);
	...
	for (rBlockSize = 0; rBlockSize < blockSize; rBlockSize++) {
		if (rNBlock < dis(rBlockSize)) {
			for (posInNgram = 0; posInNgram < n - 1; posInNgram++) {
				cout << inputWordsByIndex[data[pos(rBlockSize) + posInNgram]]
						<< " ";
			}
			cout << outputWordsByIndex[data[pos(rBlockSize) + n - 1]] << endl;

		} else {
			for (posInNgram = 0; posInNgram < n; posInNgram++) {
				cout << ssArray(posInNgram) << " ";
			}
		}
	}
	//Now write only word or </s>
	for (rNBlock = n + 3; rNBlock < outMBlock; rNBlock += n + 3) {
		for (rBlockSize = 0; rBlockSize < blockSize; rBlockSize++) {
			if (rNBlock < dis(rBlockSize)) {
				cout << outputWordsByIndex[data[pos(rBlockSize) + rNBlock + n - 1]] << endl;
			} else {
				cout << ssArray(n - 1);
			}
		}
	...
	return blockSize * outMBlock / (n + 3);
}

int RecurrentDataSet::findPos(intTensor& pos, intTensor& dis) {
...
int RecurrentDataSet::readCoBiNgram(ioFile* iof) {
	int readLineNumber = 0;
	int outMBlock;
	int rNBlock = 0;
	int i;
	...
	int offset = N - n;
	if (offset < 0) {
		cout << "RecurrentDataSet::readCoBiNgram order in id file is too small"
				<< endl;
		exit(1);
	}
	int readTextNgram[N];
	...
	for (rNBlock = 1; rNBlock < outMBlock; rNBlock++) {
		for (rBlockSize = 0; rBlockSize < blockSize; rBlockSize++) {
			...
			for (i = 0; i < n - 1; i++) {
				data[ngramNumber * (n + 3) + i] = data[(ngramNumber - blockSize)
						* (n + 3) + i + 1];
			}
...
float RecurrentDataSet::computePerplexity() {
	perplexity = 0;
...
int RecurrentDataSet::addLine(ioFile* iof) {
	string line;
	if (iof->getLine(line)) {
		if (!checkBlankString(line)) {
			line = line + " " + ES;
...
void RecurrentDataSet::shuffle(int times) {
	cout << "Please not to sort n-grams of recurrent data set" << endl;
}
