...
int
SoulVocab::getHashValue(string word)
{
  int hashValue = 0;
  int idWord;
  for (idWord = 0; idWord < word.size(); idWord++)
    {
      hashValue = (256 * hashValue + (unsigned short) word[idWord]) % tableSize;
    }
  return hashValue;
}

int
SoulVocab::add(string word, int idWord)
{
  int i;
...
int
SoulVocab::add(string word, int idWord, int hashValue)
{
  int i;
  if (index(word) != ID_UNK)
    {
...
int
SoulVocab::index(string inWord)//Get index
{
  string word;
  word = inWord;
  int i;
